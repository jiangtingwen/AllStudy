...
import { Toast, install } from 'mand-mobile'
import { stringify } from 'qs'   //qs   序列化参数
import store from '../store/index'

let tmpTrip = store.state.trip

axios.defaults.timeout = 10000  //设置接口请求时间
axios.defaults.withCredentials = false  //跨域请求时需要使用凭证，默认为false

//给请求添加拦截器
axios.interceptors.request.use(
    config => {
        //每次发送请求之前判断一下vuex中是否存在token
        //如果存在,则统一在http请求的header上都加上token,
        // 这样后台能根据token判断用户当前是否是登录状态
        //即使本地存在token,也有可能token是过期的,所以在拦截前中要返回状态进行判断
        //const token = store.state.token
        //token&&(config.header.Authorization=token)
        config.header['Content-Type'] = 'application/x-www-form-urlencoded' //定义请求头,默认的提交方式
        return config
    },
    error => {
        Toast.failed('请求过程出错')
